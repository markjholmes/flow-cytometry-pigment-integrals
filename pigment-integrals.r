...
channels.dat <- read.csv(paste0(wdir, 'channels.csv')) %>% 
  ...
  # dplyr::filter(ID == 'Yellow-B' | ID == 'Red-R' | ID == 'Red-B')

### channel plot colours =====
# channels.cols <- setNames(viridis::viridis(3), c('Yellow-B', 'Red-R', 'Red-B'))

#### plot ====
ggplot() +
  geom_rect(data = channels.dat, 
            aes(xmin = LambdaCentre - LambdaPM, xmax = LambdaCentre + LambdaPM,
                fill = ID, ymin = 0, ymax = 2), alpha = .2, color = NA) +
  geom_vline(data = lasers.dat, aes(xintercept = Lambda), col = c('blue', 'red'), lwd = 1) + 
